...
def adpo_policy_loss(
    ...
    **kwargs
    ...
    # OPO Config
    # OPO Config
    opo_mu = _cfg("opo_mu", 1.0)
    opo_direction_scale = _cfg("opo_direction_scale", None)  # Optional manual scale for term1 (default: 1/alpha)
    ...
    if loss_variant == "plackett_luce":
        ...
        if loss_variant == "decoupled":
            ...
            # Term 1: -q * u
            term1 = -(1.0 / alpha) * (mix_prob.detach() * seq_log_prob_grouped).sum(dim=-1)
            ...
            # L = - (1/alpha) E[w * log π] + (mu/2) E[Δ²]
            # 
            # Mathematical Note: OPO is pointwise (not listwise).
            # We use (P, G) structure only for efficient group operations:
            #   - q = softmax(adv/β) requires group structure
            #   - w normalization requires group structure
            # The loss itself is conceptually: mean over all samples.
            # =====================================================
            alpha = effective_alpha
            eps = 1e-10

            # p_theta = softmax(u) over group
            p_theta = F.softmax(u_grouped, dim=-1)  # (P, G)

            # w = q^α * p^(1-α)
            w = (
                torch.pow(q_grouped.detach() + eps, alpha)
                * torch.pow(p_theta.detach() + eps, 1.0 - alpha)
            )

            # Group normalization on w (sampling geometry only)
            # This ensures sum(w) = 1 per group, preserving orthogonality
            w = w / (w.sum(dim=-1, keepdim=True) + eps)

            # Grouped tensors for computation
            delta = log_ratio[sorted_idx].view(num_prompts, num_generations)
            seq_log_prob_grouped = seq_log_prob[sorted_idx].view(num_prompts, num_generations)
            ...
            # Note: .mean(dim=-1) averages over G, then we average over P
            term2 = 0.5 * opo_mu * (delta ** 2).mean(dim=-1)

            loss = (term1 + term2).mean()

            # Metrics
            log_Z = torch.logsumexp(u_grouped, dim=-1)
            term_alignment = (q_grouped.detach() * u_grouped).sum(dim=-1).mean().detach()
            term_logsumexp = torch.exp(log_Z).mean().detach() / num_generations
            u_center_val = u_center.mean().detach()
    ...
    with torch.no_grad():
        ...
        metrics = {
            ...
            "adpo/opo_direction_scale": float(opo_direction_scale) if opo_direction_scale is not None else float(1.0/effective_alpha.item() if isinstance(effective_alpha, torch.Tensor) else 1.0/effective_alpha),
        }
